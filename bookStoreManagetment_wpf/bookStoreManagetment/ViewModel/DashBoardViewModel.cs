...
namespace bookStoreManagetment.ViewModel
{
    public class DashBoardViewModel : BaseViewModel
    {
        ...
        public DashBoardViewModel()
        {
            ...
            int revenue = DataProvider.Ins.DB.profitSummaries.Where(p => p.sellDay.Month == DateTime.Now.Month).Select(p => p.profit).Sum();
            Revenue = string.Format(new CultureInfo("vi-VN"), "{0:0,0 đ}", revenue);

            NewOrder = DataProvider.Ins.DB.sellBills.Count().ToString();

            BillPayment = DataProvider.Ins.DB.importBills.Count().ToString();
            ...
            var records = DataProvider.Ins.DB.profitSummaries.GroupBy(p => p.sellDay).Select(pa => new { Day = pa.Key, Sum = pa.Sum(s => s.profit) }).OrderByDescending(c => c.Day).Take(7);
